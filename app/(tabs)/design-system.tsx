import { Pressable, View, TextInput, Switch } from 'react-native';
...
function Separator() {
  ...
  return (
    <View className="my-4 h-px" style={{ backgroundColor: Colors[colorScheme].icon + '33' }} />
  );
}
...
function Badge({
  ...
  const variants = {
    default: { bg: 'bg-brand', text: 'text-white' },
    secondary: {
      bg: isDark ? 'bg-[#1a2236]' : 'bg-brand-tint',
      text: isDark ? 'text-white' : 'text-brand',
    },
    accent: { bg: 'bg-accent', text: 'text-navy' },
    destructive: { bg: 'bg-ph-red', text: 'text-white' },
    outline: { bg: 'bg-transparent', text: isDark ? 'text-white' : 'text-navy' },
  };
  ...
  return (
    ...
    >
      <ThemedText className={`text-xs ${v.text}`} style={{ fontFamily: LexendFonts.semibold }}>
        {children}
      </ThemedText>
    </View>
  );
...
function DesignButton({
  ...
  const variants = {
    default: { bg: 'bg-brand', text: 'text-white', pressed: Brand.primaryDark },
    secondary: {
      bg: isDark ? 'bg-[#1a2236]' : 'bg-brand-tint',
      text: isDark ? 'text-white' : 'text-brand',
      pressed: isDark ? '#232d47' : Brand.primaryLight,
    },
    accent: { bg: 'bg-accent', text: 'text-navy', pressed: Brand.accentDark },
    outline: {
      bg: 'bg-transparent',
      text: isDark ? 'text-white' : 'text-brand',
    ...
    destructive: { bg: 'bg-ph-red', text: 'text-white', pressed: '#a50e1f' },
  };
  ...
  return (
    ...
    >
      <ThemedText
        className={`text-center text-sm ${v.text}`}
...
export default function DesignSystemScreen() {
  ...
  return (
    <ParallaxScrollView
      headerBackgroundColor={{ light: Brand.primary, dark: '#0a1230' }}
      headerImage={
        <>
          <IconSymbol
            size={280}
            ...
            className="absolute bottom-[-40px] left-[-20px] opacity-15"
          />
    ...
    >
      ...
      <ThemedText className="mb-4 leading-5 opacity-70">
        UI components styled with NativeWind, matching the eGovPH super app — royal blue with
        Philippine flag accents and the Lexend typeface.
      </ThemedText>
      ...
      <Card className="gap-2">
        <ThemedView className="flex-row justify-between">
          ...
          <ThemedText className="text-sm text-brand" style={{ fontFamily: LexendFonts.semibold }}>
            67%
          </ThemedText>
        </ThemedView>
        <Progress value={67} />
